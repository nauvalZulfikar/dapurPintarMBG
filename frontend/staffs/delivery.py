...
from frontend.staffs.common import validate_delivery, process_delivery_scan
...
st.set_page_config(page_title="Delivery", layout="centered")
...
def handle_scan():
    ...
    ok, reason = validate_delivery(scan)

    if not ok:
        local_enqueue_error(scan, "Delivery", reason)
        st.session_state.message = ("error", f"GAGAL\n{when}\n{reason}")
        st.session_state.sound = "failed.mp3"
    else:
        local_enqueue_scan(scan, "Delivery", "delivered")
        result = process_delivery_scan(scan)
...
if st.session_state.message:
    ...
    if "delivery_result" in st.session_state:
        st.write(st.session_state.delivery_result)
