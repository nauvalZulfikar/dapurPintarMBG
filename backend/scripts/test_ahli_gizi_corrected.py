"""Test new Ahli Gizi features with corrected endpoint signatures.

Corrected endpoints based on actual implementation:
1. GET /api/menu/substitutes?food_name=BERAS — food substitutes with similarity score
2. GET /api/nutrition/daily?date=2026-04-25 — daily nutrition summary
3. GET /api/nutrition/weekly-compliance?week_start=... — weekly nutrition compliance
...
def run():
    ...
    print(f"\n[Ahli Gizi Features - Corrected] Testing with actual endpoint signatures")

    # ── Test 1: GET /api/menu/substitutes?food_name=BERAS ──────────────────────
    print("\n[1] Menu substitutes (similarity scoring)")
    r = hit("GET", "/api/menu/substitutes", token, params={"food_name": "BERAS"})
    fails += check("GET /api/menu/substitutes?food_name=BERAS → 200", r.status_code == 200, str(r.status_code))
    if r.status_code == 200:
        data = safe_json(r)
        if data:
            fails += check("response has 'food_name' field", "food_name" in data, str(list(data.keys())))
            substitutes = data.get("substitutes", [])
            fails += check("response has 'substitutes' array", isinstance(substitutes, list), str(type(substitutes)))
            fails += check("substitutes list has entries", len(substitutes) > 0, f"got {len(substitutes)} items")
            if substitutes:
                first_item = substitutes[0]
    ...
    # ── Test 3: GET /api/nutrition/weekly-compliance?week_start=... ────────────
    print("\n[3] Weekly nutrition compliance")
    week_start = "2026-04-19"  # Saturday; week would be Sat-Fri or Sun-Sat depending on implementation
    r = hit("GET", "/api/nutrition/weekly-compliance", token, params={"week_start": week_start})
    fails += check("GET /api/nutrition/weekly-compliance?week_start=... → 200", r.status_code == 200, str(r.status_code))
    if r.status_code == 200:
        data = safe_json(r)
        if data:
            fails += check("response has 'week_start' field", "week_start" in data, str(list(data.keys())))
            fails += check("response has 'week_end' field", "week_end" in data, str(list(data.keys())))
            days = data.get("days", [])
            fails += check("response has 'days' array", isinstance(days, list), str(type(days)))
            fails += check("days array has entries", len(days) >= 1, f"got {len(days)} days")
            if len(days) > 0:
                first_day = days[0]
    ...
    if r.status_code == 200:
        ...
        if data:
            ...
            fails += check("response has 'menus' array (not 'saved_menus')", isinstance(menus, list), str(type(menus)))
    ...
    if menu_id:
        ...
        if r.status_code == 200:
            data = safe_json(r)
            if data:
                remaining = [m for m in data.get("menus", []) if m.get("id") == menu_id]
...
def main():
    ...
    if fails:
        ...
        return 1
    print(f"\n[DONE] Ahli Gizi features test (corrected) passed")
    return 0
